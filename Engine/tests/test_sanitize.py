...
def test_runner_error_field_uses_class_name_only():
    ...
    err = FakePipelineError("leaked user input: [REDACTED-NAME]-DD-MMM-YYYY")
    ...
    assert "[REDACTED-NAME]" not in out["error"]
    assert "DD-MMM-YYYY" not in out["error"]
...
def test_hash_oid_stable_and_short():
    ...
    a = hash_oid("[REDACTED-NAME]-DD-MMM-YYYY-9376")
    b = hash_oid("[REDACTED-NAME]-DD-MMM-YYYY-9376")
...
def test_hash_oid_does_not_leak_order_id():
    ...
    oid = "[REDACTED-NAME]-DD-MMM-YYYY-9376"
    h = hash_oid(oid)
