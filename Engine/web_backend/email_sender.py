...
def send_post_payment_email(*, to: str, reading_url: str, order_id: str) -> None:
    ...
    # hash_oid is name-irreversible (12-char hex SHA-256 prefix). The
    # raw order_id is generated by _make_slug from name+DOB, so its
    # first 8 chars deterministically encode the customer's PII —
    # sending that to Resend (third-party) would leak name-derivation
    # to their logs and analytics surfaces. The hash is what we use
    # everywhere else for log correlation; same value here keeps a
    # single ID for support to grep.
    order_id_short = hash_oid(order_id)
    try:
        resend.Emails.send({
            "from": sender,
            "to": to,
