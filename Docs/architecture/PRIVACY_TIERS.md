...
*Doc version 1.3 — 2026-04-XX — P2G-followup closure: migration script hardened (V2 full-literal preservation, V3 three-state classifier covering both missing-`order_id`-with-PII-keys and `order_id`↔`path.stem` mismatch via `SuspiciousRowAbort` + exit code 3) and test #17 rewritten with sentinel `os.getpid` to actually observe `_atomic_write_json` (Codex R6 C2 + R2 C1 catches). Suite 236 → 239. Two §16.5 surfaces remain deferred with ~10⁵-active-order revisit thresholds: `hash_oid` 12-char log-correlation truncation, and `_make_slug` 4-char name+DOB-encoded order_id (filename + row field). See `Docs/engine/SIRR_MASTER_REGISTRY.md` §16.5 for upgrade paths.*
